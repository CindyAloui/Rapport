...
Dans de nombreuses langues dont le français un nom peut être comptable où massif. Les noms comptable peuvent être "comptés" comme leurs noms l'indique, ils peuvent être au singulier ou au pluriel, par exemple : un stylo, deux stylos. Les noms massifs eux ne peuvent être comptés, ils ont donc seulement une forme singulier comme eau ou sable.
...
Nous pensons que connaître seulement la classe lexicale d'un mot n'est pas suffisant, en effet, il y a le problème des mots ambigus et aussi parfois des mot typiquement massif ou comptable peuvent être employé dans des contextes ne correspondant pas à leurs classes lexical, par exemple : "Il s'agit d'une \textbf{eau} contenant beaucoup de minéraux", cependant la classe du "contexte" ne suffit pas non plus, car un grand nombre de contextes ne sont pas discriminants, on pense donc avoir besoin pour répondre à un plus grand nombre de problèmes de pouvoir classifier un certain mot dans un certain contexte. 

Nous souhaitons aussi que notre système soit généralisable à d'autres classes sémantique telle que concret/abstrait ou animé/inanimé, car cela permettrait de désambiguïser plus de mots, pour cela, il faudrait que notre système soit très général et non pas spécialement adapté à la tâche de classification comptable/massif et n'ai besoin que d'un minimum de données supervisé pour que l'on puisse l'entraîner pour d'autres classes facilement.\\
\\
\\
\textbf{Expliquer le plan du mémoire}

\section{État de l'art}

Comme in existe un certain nombre d'applications, plusieurs chercheurs ont travaillé sur le problème de la comptabilité (pour l'anglais). Baldwin and Bond (2003a; 2003b) ont proposé une méthode pour apprendre la comptabilité des mots hors contexte à partir de données d'un corpus annoté. Lapatta et Keller (2005) et Pend et Araki (2005) ont proposé des modèles Web pour apprend la comptabilité. \textbf{à complété}

\`A notre connaissance il n'y a pas de travaux sur la comptabilité des mots français ni de modèle utilisant peut de données annotées et cherchant un modèle généralisable, en effet dans la majorité des travaux portant sur la comptabilité des mots utilisent des informations tel que la proportion d'apparition d'un mot au pluriel ainsi que la proportion d'apparition du mot précédé par un article indéfini qui sont des informations très adaptés à cette tache en particulier et ne permet donc pas de généraliser à d'autres classes sémantique. De plus la plupart des travaux tentent uniquement de définir la comptabilité des mots hors contexte hors, c'est pour ces raisons là que notre travail diffère fortement des autres travaux effectués sur ce sujet. 
